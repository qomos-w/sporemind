...
func TestManifestNormalize(t *testing.T) {
	...
	if n.SdkVersion != Version {
		t.Fatalf("expected SdkVersion default %q, got %q", Version, n.SdkVersion)
	}
	if n.Namespace != m.ID {
		t.Fatalf("expected Namespace %q, got %q", m.ID, n.Namespace)
	}
	// An explicit SdkVersion survives Normalize (older SDK build stamp).
	m.SdkVersion = "0.2.9"
	if got := m.Normalize().SdkVersion; got != "0.2.9" {
		t.Fatalf("explicit SdkVersion must survive Normalize, got %q", got)
	}
}
...
func TestToAppManifest(t *testing.T) {
	...
	if app.SdkVersion != Version {
		t.Fatalf("expected SdkVersion %q, got %q", Version, app.SdkVersion)
	}
