...
export function detectDockZone(
  ...
  if (relX < DOCK_THRESHOLD)      return relY < hExcludingBottom / 2 ? 'left-top'    : 'left-bottom'
  if (relX > w - DOCK_THRESHOLD)  return relY < hExcludingBottom / 2 ? 'right-top'   : 'right-bottom'
  if (relY > h - DOCK_THRESHOLD)  return relX < w / 2 ? 'bottom-left' : 'bottom-right'
  return null
}
