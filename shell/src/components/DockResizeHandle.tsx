...
export const DockResizeHandle: React.FC<{
  direction: 'horizontal' | 'vertical'
  onResize: (delta: number) => void
  className?: string
  /** Optional: direct DOM target to resize. When set, drag updates DOM directly
   *  via RAF; onResize is called once on mouseup to sync React state. */
  targetRef?: React.RefObject<HTMLElement>
  /** When in targetRef mode, negate the delta (for right/bottom side panels
   *  where dragging outward should shrink the panel). */
  negateDelta?: boolean
}> = ({ direction, onResize, className, targetRef, negateDelta }) => {
  ...
  const scheduleDOMUpdate = useCallback((currentPos: number) => {
  ...
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    ...
    if (targetRef?.current) {
      ...
      const startRef: { current: number | null } = { current: startPos }
  ...
  return (
    <div
      className={`shell-resize-handle ${direction} ${className || ''}`}
