...
func TestHTTPClientForParams_ProxyDialsThroughProxy(t *testing.T) {
	c, err := httpClientForParams(Params{Proxy: "http://videogen-proxy-test:3128"})
	if err != nil {
	...
	if u == nil || u.Host != "videogen-proxy-test:3128" {
		t.Fatalf("transport must dial through videogen-proxy-test:3128, got %v", u)
	}
}
